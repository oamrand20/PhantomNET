...
function usersController() {
  ...
  useEffect(() => {
    ...
    if (!searching) {
      setIsLoading(true);
      // VARS
      const url = `${db_link}/api/files?page=${page}`;
      // Function
      axios
        .get(url, {
          headers: {
  ...
  useEffect(() => {
    ...
    if (searching) {
      ...
      const url = `${db_link}/api/search/files?page=${pageSearch}`;
      // Function
      axios
        .post(
          url,
  ...
  const handleSearch = (e: any) => {
    ...
    setIsLoading(true);
    // VARS
    const url = `${db_link}/api/search/files?page=${page}`;
    ...
    >
      <div className="bg-white rounded-lg shadow-sm">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center gap-4 p-6 border-b">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">{t.files}</h1>
            <p className="text-sm text-gray-500 mt-1">{t.thisIsAllFiles}</p>
          </div>
